import {mostrartarjeta,mostrarbox, filtradoPorNombre, cualca, manejarSearch,separarCategorias,filtradop} from "../modules/functions.js";
...
.then(response=>{
    ...
    let eventoPasado = filtradop(data,date)
    
    mostrartarjeta(eventoPasado)
    separarCategorias(eventoPasado)
    arrayDeCajitas.addEventListener("change", ()=>manejarSearch(eventoPasado))
    search.addEventListener("input",()=>manejarSearch(eventoPasado))

}).catch(error => {console.log('Error al obtener los datos de la API:', error);
})
/*
function filtradop(filtro,data){
    let pasado=[]
...
for(let categoria of eventoPasado){
 ...
 function manejarSearch(){
     filtrado de search 
     let filtradoNombreEvento = filtradoPorNombre(eventoPasado,search.value)
     ...
     let igualados = eventoPasado.filter(elemen => arrayValoresCheck.includes(elemen.category))
